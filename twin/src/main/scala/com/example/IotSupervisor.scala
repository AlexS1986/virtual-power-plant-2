...
import _root_.com.example.projections.DeviceTemperatureProjection
...
object IotSupervisor {
  ...
  def apply(httpPort: Int): Behavior[IotSupervisor.Command] = {
    Behaviors.setup[IotSupervisor.Command](context => {
      ...
      val deviceTemperatureRepository = new DeviceTemperatureRepositoryImpl() // connection to database
      DeviceTemperatureProjection.init(context.system,deviceTemperatureRepository)
