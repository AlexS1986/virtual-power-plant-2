...
object Device {
  ...
  // these could also live elsewhere and the Device class be completely
  // oblivious to being used in sharding
  def initSharding(system: ActorSystem[_]): Unit = { // inits sharding with tags for projection
    ...
    /*ClusterSharding(system).init(Entity(TypeKey) { entityContext =>
      ...
      Device(entityContext.entityId, persistenceId)
    })*/
  }
